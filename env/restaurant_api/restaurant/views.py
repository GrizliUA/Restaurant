...
class RestaurantUpdateDestroyAPIList(generics.RetrieveUpdateDestroyAPIView):
    queryset = Restaurant.objects.all()
    serializer_class = RestaurantSerializer
    permission_classes = (IsAdminOrReadOnly,)  # Only admin users can modify or delete
...
def today_menu(request):
    ...
    a = Menu.objects.all().filter().order_by('-menu_vote_count').first()
    # print(a)
    # print("Asd")
    most_voted_menu = Menu.objects.filter().order_by('-menu_vote_count').first()
    print(most_voted_menu.pk)
